...
function getAllMovies(req, res) {
    models.MOVIES.find({}).exec(function (err, resu) {
        if (resu) {
...
function getMovie(req, res) {
    models.MOVIES.findById(req.params.id).exec(function (err, resu) {
        if (resu) {
            res.status(200).send({ data: resu });
        } else {
            res.status(500).send({ message: "Error al Consultar" });
...
function removeMovie(req, res) {
    ...
    models.MOVIES.findById(moviesId).remove(function (err, resu) {
        if (!err) {
            models.THEATERSXMOVIES.find({ MOVIEID: moviesId }).remove(function (err, resu) {
                if (!err) {
